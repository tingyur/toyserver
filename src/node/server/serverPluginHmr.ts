...
export const debugHmr = require('debug')('toyserver:hmr')
...
export const hmrPlugin: ServerPlugin = ({
  ...
  server.on('upgrade', (req, socket, head) => {
    if (req.headers['sec-websocket-protocol'] === 'toyserver-hmr') {
  ...
  wss.on('error', (e: Error & { code: string }) => {
    if (e.code !== 'EADDRINUSE') {
      console.error(chalk.red(`[toyserver] WebSocket server error:`))
      console.error(e)
    }
  })
  ...
  const checkHotCall = (
    ...
    if (
      ...
      if (isTopLevel) {
        ...
        console.warn(
          chalk.yellow(
            `[toyserver] HMR syntax error in ${importer}: import.meta.hot.accept() ` +
      ...
      if (method === 'accept' || method === 'acceptDeps') {
        if (!isDevBlock) {
          console.error(
            chalk.yellow(
              `[toyserver] HMR syntax error in ${importer}: import.meta.hot.${method}() ` +
        ...
        if (accepted && accepted.type === 'ArrayExpression') {
          if (method !== 'acceptDeps') {
            console.error(
              chalk.yellow(
                `[toyserver] HMR syntax error in ${importer}: hot.accept() only accepts ` +
          ...
          accepted.elements.forEach((e) => {
            if (e && e.type !== 'StringLiteral') {
              console.error(
                chalk.yellow(
                  `[toyserver] HMR syntax error in ${importer}: hot.accept() deps ` +
          ...
          if (method !== 'accept') {
            console.error(
              chalk.yellow(
                `[toyserver] HMR syntax error in ${importer}: hot.acceptDeps() ` +
          ...
          console.error(
            chalk.yellow(
              `[toyserver] HMR syntax error in ${importer}: ` +
